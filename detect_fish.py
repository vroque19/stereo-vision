        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for c in det[:, 5].unique():
                    ...
                    s += f"{n} fish{'s' * (n > 1)}, "  # add to string

                # Write results
                for *xyxy, conf, cls in reversed(det):
                    c = int(cls)  # integer class
                    label = fish if hide_conf else f"fish"
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        label = (
                            None
                            if hide_labels
                            else ("fish" if hide_conf else f"fish {conf:.2f}")
                        )
                    ...
                    if save_crop:
                        save_one_box(
                            xyxy,
                            imc,
                            file=save_dir / "crops" / "fish" / f"{p.stem}.jpg",
                            BGR=True,
                        )
...
def parse_opt():
    ...
    parser.add_argument(
        "--data",
        type=str,
        default=ROOT / "data/Objects365.yaml",
